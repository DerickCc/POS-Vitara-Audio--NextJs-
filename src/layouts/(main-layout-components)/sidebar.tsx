...
export default function Sidebar({ className }: { className?: string }) {
  ...
  return (
    <aside
      className={cn(
        'fixed bottom-0 start-0 z-50 h-full w-[270px] border-e-2 border-gray-100 bg-white 2xl:w-72 dark:bg-gray-100/50 shadow',
        className
      )}
    >
                    ...
                    {item?.dropdownItems ? (
                      ...
                      >
                        {item?.dropdownItems?.map((dropdownItem, index) => {
                          const isChildActive = pathname.includes(dropdownItem?.href as string);
