...
export const searchSo = async (code?: string, progressStatus?: string): Promise<any[]> => {
  try {
    const response = await apiFetch(`/api/sales-orders/search${toQueryString({ code, progressStatus })}`, { method: 'GET' });
    return response.result;
  } catch (e) {
    throw e + '';
  }
